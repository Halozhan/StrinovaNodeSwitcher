...
namespace WpfApp.Services
{
    public class RegionFactory
    {
        ...
        public async Task LoadNodesAsync(string regionName, string regionCode)
        {
            ...
            // EdgeOne Accelerator
            var edgeOneResult = await _client.QueryAsync($"klbq-prod-ds-{regionCode}1-eo.strinova.com", QueryType.A);
            ...
            for (int i = 1; i <= 150; i++)
            {
                var serverResult = await _client.QueryAsync($"klbq-prod-ds-{regionCode}{i}-server.strinova.com", QueryType.A);
